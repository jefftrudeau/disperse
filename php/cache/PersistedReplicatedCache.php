...
require_once(Disperse::getHome().'/php/cache/ReplicatedCache.php');

/**
 * Extends ReplicatedCache and provides a persistent storage mechanism for cached data through the
 * use of Datasource and its accompanying objects.
 * 
 * @package Disperse
 */
class PersistedReplicatedCache extends ReplicatedCache implements Cacheable {
  ...
  private function lock($key, $table) {
    while ($lock = parent::get($table.'_'.$key, 'lock')) {
      usleep(Disperse::getCacheRetryInterval());
    }
